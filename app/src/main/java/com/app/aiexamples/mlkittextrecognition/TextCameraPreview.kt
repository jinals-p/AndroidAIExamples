...
@Composable
fun TextCameraPreview(
    modifier: Modifier = Modifier,
    torchEnabled: Boolean = false,
    isFrozen: Boolean = false,
    onTextDetected: (fullText: String, blocks: List<String>) -> Unit
) {
    ...
    AndroidView(
        ...
        modifier = modifier,
        update = { view ->
            if (!isFrozen) {  // ✅ stop processing when frozen
                startTextCamera(
                    context = context,
    ...
    cameraProviderFuture.addListener({
        ...
        try {
            ...
            val camera = cameraProvider.bindToLifecycle(
                lifecycleOwner,
                CameraSelector.DEFAULT_BACK_CAMERA,  // back camera for text
                preview,
                imageAnalyzer
            )
